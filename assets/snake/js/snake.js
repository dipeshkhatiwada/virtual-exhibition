...
SNAKE.Snake = SNAKE.Snake || (function() {
    ...
    */
    return function(config) {
        alert(localStorage.jsSnakeHighScore);
        if (!config||!config.playingBoard) {return;}
        if (localStorage.jsSnakeHighScore == 'undefined') localStorage.setItem('jsSnakeHighScore', document.getElementById('high_score').value);
...
SNAKE.Board = SNAKE.Board || (function() {
    ...
    */
    function getClientWidth(){
        var myWidth = document.getElementById('snake_board_box').offsetWidth;
    ...
    */
    function getClientHeight(){
        var myHeight = 0;
       var myHeight = document.getElementById('snake_board_box').offsetWidth;
        return myHeight;
    }
    ...
    return function(inputConfig) {
        ...
        function createBoardElements() {
            ...
            elmAboutPanel.innerHTML = 'Total Attempts: '+document.getElementById('total_attempts').value;

            elmLengthPanel = document.createElement("div");
            elmLengthPanel.className = "snake-panel-component";
            elmLengthPanel.innerHTML = "Rank: "+document.getElementById('rank').value+'/'+document.getElementById('total_participant').value;

            elmHighscorePanel = document.createElement("div");
            elmHighscorePanel.className = "snake-panel-component";
            elmHighscorePanel.innerHTML = "Highscore: " + localStorage.jsSnakeHighScore+'/'+document.getElementById('overal_high_score').value;
        ...
        function updateDatas() {
            ...
            var prv_attempt = document.getElementById('total_attempts').value;
            var prv_score = document.getElementById('high_score').value;
            
            var total_attempts = Number(prv_attempt) + Number(1);
            var high_score = prv_score;
            if (Number(prv_score) < Number(localStorage.jsSnakeHighScore)) {
                high_score = localStorage.jsSnakeHighScore;
            }
          
            document.getElementById('high_score').value = high_score;
            document.getElementById('total_attempts').value = total_attempts;
            $.ajax({
                url: $('#update_score').attr('action'),
                type: 'post',
                dataType: 'json',
                data: new FormData($('#update_score')[0]),
                cache: false,
                contentType: false,
                processData: false,
                ...
                success: function(json) {
                    if (json['error']) {
                        ...
                        document.getElementById('overal_high_score').value = json['overal_high_score'];
                        document.getElementById('rank').value = json['rank'];
                        document.getElementById('total_participant').value = json['total_participant'];
                        elmHighscorePanel.innerHTML = "Highscore: " + high_score+'/'+json['overal_high_score'];
                        elmAboutPanel.innerHTML = 'Total Attempts: '+total_attempts;
                        elmLengthPanel.innerHTML = "Rank: "+json['rank']+'/'+json['total_participant'];
                    }
                },
        ...
        */
        me.setupPlayingField = function () {
            ...
            elmAboutPanel.style.width = "250px";
            elmAboutPanel.style.left = Math.round(cWidth/2) - Math.round(350/2) + "px";

            elmLengthPanel.style.top = pLabelTop;
            elmLengthPanel.style.left = 30 + "px";

            elmHighscorePanel.style.top = pLabelTop;
            elmHighscorePanel.style.left = cWidth - 240 + "px";
        ...
        */
        me.foodEaten = function() {
            ...
            document.getElementById('high_score').value = localStorage.jsSnakeHighScore;
