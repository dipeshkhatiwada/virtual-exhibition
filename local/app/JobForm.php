...
class JobForm extends Model
{
    ...
    {
    	$title = '';
    	$form = JobForm::select('f_lable')->where('id',$id)->first();
    ...
    public static function createForm($id,$title,$type,$list,$rq,$marks,$extra,$extra_type,$fe_lable){
			...
			if (in_array($type, $intypes)) {
   				...
   				$form='<input type="hidden" name="my_datas['.$id.'][requ]" value="'.$rq.'"><input type="'.$type.'" class="form-control" name="my_datas['.$id.'][fdata]"  '.$req.' /> <input type="hidden" name="my_datas['.$id.'][marks]" value="'.$marks.'">';
				return $form;
				}
			elseif(in_array($type, $rtypes)) {
				...
				$form = '';
				foreach($lm as $l){
   				$form +='<input type="hidden" name="my_datas['.$id.'][requ]" value="'.$rq.'"><input type="'.$type.'" class="form-control" name="my_datas['.$id.'][fdata]" id="'.$l.'" value="'.$title.'" '.$req.' /><input type="hidden" name="my_datas['.$id.'][marks]" value="'.$marks.'">&nbsp &nbsp $nbsp'.$title;
				
				}
				return $form;
				}
			elseif($type =='textarea'){
				$form='<input type="hidden" name="my_datas['.$id.'][requ]" value="'.$rq.'"><textarea class="form-control" name="my_datas['.$id.'][fdata]"></textarea> <input type="hidden" name="my_datas['.$id.'][marks]" '.$req.' value="'.$marks.'">';
				return $form;
				}
			elseif($type =='file'){
				$form='<input type="hidden" name="extrafile['.$id.'][requ]" value="'.$rq.'"><input type="'.$type.'" class="form-control" name="extrafile['.$id.'][file]"  '.$req.' /><input type="hidden" name="extrafile['.$id.'][marks]" '.$req.' value="'.$marks.'">';
				return $form;
				}
			elseif($type =='select'){
				$form = '<input type="hidden" name="my_datas['.$id.'][requ]" value="'.$rq.'"><select id="hidden_'.$id.'" name="my_datas['.$id.'][marks]" '.$req.' style="display:none;">';
				$m=explode(',',$marks);
				foreach($m as $key => $l){
   				$form .='<option id="'.$key.'" value="'.$l.'">'.$l.'</option>';
				}
				$form .='</select><select opvl="'.$id.'" name="my_datas['.$id.'][fdata]" '.$req.' class="form-control select"><option value="">Select Option </option>';
				$lm=explode(',',$list);
				foreach($lm as $k => $ml){
					$m=explode(',',$marks);
				foreach($m as $key => $l){
				...
				$form .='<div id="aextra_'.$id.'" style="margin-top:10px;"></div>';
				return $form;
				}
			else {
				return '';
