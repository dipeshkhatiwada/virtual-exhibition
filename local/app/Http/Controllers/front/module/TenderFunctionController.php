...
public function index($id,$setting)
    {
      ...
      //dd($setting);
        ...
        //$limit =    $setting['limit'];
        $today = date('Y-m-d');
       $tenders=Tender::where('tender_function', $setting->tender_function)->where('status', 1)->where('publish_date', '<=', $today)->where('submission_end_date', '>=', $today)->orderBy('id','desc')->limit($setting->per_page)->get();
        ...
        if(count($tenders) > 0){
            ...
            $datas['href'] = url('/tenders/function/'.$setting->tender_function);
            $datas['id'] = $id;
