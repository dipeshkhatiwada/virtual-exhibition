...
<div class="col-md-12 panel-heading">
    <div class="product-link btn-group ">
        ...
        <a href="{{url('/employer/dashboard/training')}}" class="btn app_active">Training</a>
    ...
    <div id="job" class="careerfy-typo-wrap">
		...
		<div class="col-md-1 col-3">
			<a href="{{url('/employer/dashboard/training')}}" class="btn btn-block {{$data['filter_stat'] == '' ? 'whiteclr bluecomnbtn' : 'app_default'}}">Recent</a>
		</div>
		<div class="col-md-1 col-3">
			<a href="{{url('/employer/dashboard/training?filter_stat=1')}}" class="btn btn-block {{$data['filter_stat'] == '1' ? 'whiteclr bluecomnbtn' : 'app_default'}} ">Total</a>
		</div>    
		...
		<div class="col-md-3 tp5p">
			<span class="greenclr lft15p">Training analytics by date.</span>
		</div>
	</div>
	...
	<div class="row tp10p cm10-row">	
		<div class="col-lg-2 col-md-2 col-xs-6 col-4">
			...
			<div class="small-box bluegradient">
				<div class="inner">
				<h3>{{$data['total_training']}}</h3>
				<p>Training Post</p>
				</div>
				<div class="icon">
				<i class="ion ion-stats-bars"></i>
				</div>
				<a href="{{url('/employer/training')}}" class="small-box-footer">More info <i class="fa fa-arrow-circle-right"></i></a>
			</div>
		</div>
        <div class="col-lg-2 col-md-2 col-xs-6 col-4">
          <!-- small box -->
			...
			<div class="dash_com_block">
				<ul class="nav nav-tabs dash_tab" id="myTab" role="tablist">
					<li class="nav-item tablink">
						<a class="nav-link active" id="active_job" data-toggle="tab" href="#activejob" role="tab" aria-controls="activejob" aria-selected="true">Active Training</a>
					</li>
					<li class="nav-item tablink">
						<a class="nav-link" id="pending_job" data-toggle="tab" href="#pendingjob" role="tab" aria-controls="pendingjob" aria-selected="false">Pending Training</a>
					</li>
					<li class="nav-item tablink">
						<a class="nav-link" id="expired_job" data-toggle="tab" href="#expiredjob" role="tab" aria-controls="expiredjob" aria-selected="false">Expired Training</a>
					</li>
				...
				<div class="tab-content businesstab flex">							
					<div class="tab-pane active" id="activejob" role="tabpanel" aria-labelledby="active_job">
							...
							<div class="careerfy-managejobs-list">
								...
								<div class="careerfy-table-layer careerfy-managejobs-thead">
									...
									<div class="careerfy-table-row">
										...
										<div class="careerfy-table-cell">Address</div>
										<div class="careerfy-table-cell">Opening Date</div>
										<div class="careerfy-table-cell">Closing Date</div>
								...
								<div class="careerfy-table-layer careerfy-managejobs-tbody">
									...
									<div class="careerfy-table-row">
										...
										<div class="careerfy-table-cell">{{$active->venue}}</div>
										<div class="careerfy-table-cell">{{$active->address}}</div>
										<div class="careerfy-table-cell">{{$active->start_date}}</div>
										<div class="careerfy-table-cell">{{$active->end_date}}</div>
										<div class="careerfy-table-cell"><a href="{{url('/employer/training/apply/'.$active->id)}}" class="careerfy-managejobs-appli">{{count($active->Apply)}}</a></div>
									</div>
									@endforeach
								</div>
							</div>
						...
						You don't have any Active Training at the moment.
						<a href="{{url('employer/training/addnew')}}">
							<strong>Post a Training, now!</strong></a>
						</div>
						@endif
					</div>
							...
							<div class="careerfy-table-layer careerfy-managejobs-tbody">
								@foreach($data['pending'] as $pending)
									<div class="careerfy-table-row">
										...
										<div class="careerfy-table-cell">{{$pending->venue}}</div>
										<div class="careerfy-table-cell">{{$pending->address}}</div>
										<div class="careerfy-table-cell">{{$pending->start_date}}</div>
										<div class="careerfy-table-cell">{{$pending->end_date}}</div>
										<div class="careerfy-table-cell"><a href="{{url('/employer/training/apply/'.$pending->id)}}" class="careerfy-managejobs-appli">{{count($pending->Apply)}}</a></div>
									</div>
									@endforeach
							</div>
							</div>
						...
						<div class="careerfy-managejobs-list-wrap">
							...
							<div class="careerfy-table-layer careerfy-managejobs-tbody">
								...
								@foreach($data['completed'] as $completed)
										...
										<div class="careerfy-table-cell">{{$completed->venue}}</div>
										<div class="careerfy-table-cell">{{$completed->address}}</div>
										<div class="careerfy-table-cell">{{$completed->start_date}}</div>
										<div class="careerfy-table-cell">{{$completed->end_date}}</div>
										<div class="careerfy-table-cell"><a href="{{url('/employer/training/apply/'.$completed->id)}}" class="careerfy-managejobs-appli">{{count($completed->Apply)}}</a></div>
									</div>
									@endforeach
							</div>
							</div>
...
{
    var url = '{{url('/employer/dashboard/training?')}}'
    var from = $('#from').val();
    var to = $('#to').val();
