...
<div class="col-md-12 panel-heading">
    <div class="product-link btn-group ">
        ...
        <a href="{{url('/employer/dashboard/tender')}}" class="btn app_default">Tender</a>
         <a href="{{url('/employer/dashboard/project')}}" class="btn app_active">Project</a>
    ...
    <div id="job" class="careerfy-typo-wrap">
		...
		<div class="col-md-1 col-3">
			<a href="{{url('/employer/dashboard/project')}}" class="btn btn-block {{$data['filter_stat'] == '' ? 'whiteclr bluecomnbtn' : 'app_default'}}">Recent</a>
		</div>
		<div class="col-md-1 col-3">
			<a href="{{url('/employer/dashboard/project?filter_stat=1')}}" class="btn btn-block {{$data['filter_stat'] == '1' ? 'whiteclr bluecomnbtn' : 'app_default'}} ">Total</a>
		</div>    
		...
		<div class="col-md-3 tp5p">
			<span class="greenclr lft15p">Project analytics by date.</span>
		</div>
	</div>
	...
	<div class="row tp10p cm10-row">	
		<div class="col-lg-2 col-md-2 col-xs-6 col-4">
			...
			<div class="small-box bluegradient">
				...
				<a href="{{url('/employer/project')}}" class="small-box-footer">More info <i class="fa fa-arrow-circle-right"></i></a>
			</div>
		</div>
        <div class="col-lg-2 col-md-2 col-xs-6 col-4">
          <!-- small box -->
			...
			<div class="dash_com_block">
				<ul class="nav nav-tabs dash_tab" id="myTab" role="tablist">
					<li class="nav-item tablink">
						<a class="nav-link active" id="active_job" data-toggle="tab" href="#activejob" role="tab" aria-controls="activejob" aria-selected="true">Active Projects</a>
					</li>
					<li class="nav-item tablink">
						<a class="nav-link" id="pending_job" data-toggle="tab" href="#pendingjob" role="tab" aria-controls="pendingjob" aria-selected="false">Pending Projects</a>
					</li>
					<li class="nav-item tablink">
						<a class="nav-link" id="expired_job" data-toggle="tab" href="#expiredjob" role="tab" aria-controls="expiredjob" aria-selected="false">Expired Projects</a>
					</li>
				...
				<div class="tab-content businesstab flex">							
					<div class="tab-pane active" id="activejob" role="tabpanel" aria-labelledby="active_job">
							...
							<div class="careerfy-managejobs-list">
								...
								<div class="careerfy-table-layer careerfy-managejobs-tbody">
									...
									<div class="careerfy-table-row">
										...
										<div class="careerfy-table-cell">{{\App\ProjectCategory::getTitle($active->project_category_id)}}</div>
										<div class="careerfy-table-cell">{{$active->publish_date}}</div>
										<div class="careerfy-table-cell">{{$active->project_code}}</div>
										<div class="careerfy-table-cell"><a href="{{url('/employer/project/bids/'.$active->id)}}" class="careerfy-managejobs-appli">{{\App\ProjectApply::totalBidder($active->id)}}</a></div>
									</div>
									@endforeach
								</div>
							</div>
						...
						You don't have any Active Project at the moment.
						<a href="{{url('employer/project/addnew')}}">
							<strong>Post a Project, now!</strong></a>
						</div>
						@endif
					</div>
							...
							<div class="careerfy-table-layer careerfy-managejobs-tbody">
								@foreach($data['pending'] as $pending)
									<div class="careerfy-table-row">
										...
										<div class="careerfy-table-cell">{{\App\ProjectCategory::getTitle($pending->project_category_id)}}</div>
										<div class="careerfy-table-cell">{{$pending->publish_date}}</div>
										<div class="careerfy-table-cell">{{$pending->project_code}}</div>
										<div class="careerfy-table-cell"><a href="{{url('/employer/project/bids/'.$pending->id)}}" class="careerfy-managejobs-appli">{{\App\ProjectApply::totalBidder($pending->id)}}</a></div>
									</div>
									@endforeach
							</div>
							</div>
						...
						<div class="careerfy-managejobs-list-wrap">
							...
							<div class="careerfy-table-layer careerfy-managejobs-tbody">
								...
								@foreach($data['completed'] as $completed)
										...
										<div class="careerfy-table-cell">{{\App\ProjectCategory::getTitle($completed->project_category_id)}}</div>
										<div class="careerfy-table-cell">{{$completed->publish_date}}</div>
										<div class="careerfy-table-cell">{{$completed->project_code}}</div>
										<div class="careerfy-table-cell"><a href="{{url('/employer/project/bids/'.$completed->id)}}" class="careerfy-managejobs-appli">{{\App\ProjectApply::totalBidder($completed->id)}}</a></div>
									</div>
									@endforeach
							</div>
							</div>
...
{
    var url = '{{url('/employer/dashboard/project?')}}'
    var from = $('#from').val();
    var to = $('#to').val();
