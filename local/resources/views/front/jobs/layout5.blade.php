...
<section class="noticeblock4">
  ...
  <div class="container">
    <div class="brandNavigation3">
      <div class="row">
        <div class="col-lg-10 col-md-8 col-4">
          <nav class="navbar navbar-expand-sm brandmenu stick-top">
            ...
            <div class="collapse navbar-collapse brandNav brandNav2" id="brandNav">
              <ul class="navbar-nav nav-pills">
                <li><div class="brand_logo3">
                     @if($datas['employer_logo'] != '')
                    <img src="{{asset($datas['employer_logo'])}}" title="{{$datas['employer']->name}}" >
                  @else
      ...
      <div class="brandNav5_border_bottom"></div>
      @if(count($datas['notice']) > 0)
      <div class="tb20p">
        <h2 class="h2 center btm15m brandcolor">Notice</h2>
        <div class="slider noticeplay multipleslider">
          @foreach($datas['notice'] as $notice)
                @php($dt = \Carbon\Carbon::parse($notice->date))
          <div>
            <div class="notice5 whitebg text-ellipsis">
              <div class="triangle-topleft"></div>
            
              <h2 class="title_three" onclick="viewNoticeDetail({{$notice->id}})" style="cursor: pointer;">{{$notice->title}}</h2>
              <p><span class="brandcolor"><i class="fa fa-calendar-alt"></i></span> {{$dt->day}} {{$dt->formatLocalized('%b')}}, {{$dt->year}}</p>
            </div>
          </div>
          @endforeach
        </div>
...
The assessments vary depending of the specific function, however it will often start with a technical assessment/s followed by a competency based interview.</div>
          ...
          <div class="recommendation tp20p">
            ...
            @if($datas['job']->job_file != '')
                                    ...
                                    <a href="{{asset('/image/'.$datas['job']->job_file)}}" download class="large_bluebtn">Download ToR</a> 
                                    </div>
                                    @endif
