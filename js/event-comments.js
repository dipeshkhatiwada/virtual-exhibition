...
$(document).ready(function() {
    ...
    e.preventDefault();
		cancel_reply();
    $current = $(this);
    let comment_id = $current.attr('comment_id');
		el = document.createElement('li');
			...
			'<div class=\"col-md-12 reply_comment\">'+
				'<div class=\"row\">'+
					...
					'<div class=\"box_comment col-md-10\">'+
					  '<textarea class=\"comment_replay\" id="reply-comment" placeholder=\"Add a comment...\"></textarea>'+
					  '<div class=\"box_post\">'+
						'<div class=\"pull-right\">'+
						  ...
						  '<button onclick=\"submit_reply('+comment_id+')\" type=\"button\" value=\"1\">Reply</button>'+
						'</div>'+
					  '</div>'+
					'</div>'+
				'</div>'+
			'</div>';
		$current.closest('li').find('.child_replay').first().prepend(el);
	});
});
